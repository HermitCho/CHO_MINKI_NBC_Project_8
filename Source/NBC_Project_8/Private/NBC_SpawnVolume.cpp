...
AActor* ANBC_SpawnVolume::SpawnRandomItem()
{
    if (FNBC_FItemSpawnRow* SelectedRow = GetRandomItem())
    {
...
FNBC_FItemSpawnRow* ANBC_SpawnVolume::GetRandomItem() const
{
    if (!ItemDataTable) return nullptr;

    // 1) 모든 Row(행) 가져오기
    TArray<FNBC_FItemSpawnRow*> AllRows;
    ...
    for (const FNBC_FItemSpawnRow* Row : AllRows) // AllRows 배열의 각 Row를 순회
    {
        if (Row) // Row가 유효한지 확인
        {
    ...
    // 4) 누적 확률로 아이템 선택
    for (FNBC_FItemSpawnRow* Row : AllRows)
    {
...
AActor* ANBC_SpawnVolume::SpawnItem(TSubclassOf<AActor> ItemClass)
{
